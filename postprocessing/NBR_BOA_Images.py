...
def callNBR(nir,swir,filepath,image,resolution,nameDatum,band):
    ...
    outPath = os.path.join(filepath2,'nbr',image, 'IMG_DATA', resolution, nameDatum + 'NBR.tif')
...
def nbrOldNew(nbrDict,filepath,resolution):
    ...
    for key in nbrDict:
        if(key != len(nbrDict)):
            ...
            outPath = os.path.join(filepath,nameDatum + nameDatum2 + 'result_NBR.tif')
...
nbrOldNew(nbrDict,filepath2,"R10m")
#for image2 in arr2:
