...
def create_composites(imgfolder, Level = '2A'):
    ...
    try:
        ...
        print('Error: Creating directory', os.path.join(imgfolder,'CIR_composites'))
    ...
    for folder in subfolders:
        ...
        granule_folder = os.listdir(os.path.join(imgfolder, folder, 'GRANULE'))[0]
        if Level == '2A':
            bands_path = os.path.join(imgfolder, folder, 'GRANULE', granule_folder, 'IMG_DATA', 'R10M')
        else:
            bands_path = os.path.join(imgfolder, folder, 'GRANULE', granule_folder, 'IMG_DATA')
    ...
    print("Finished. All composites can be found in folder " + imgfolder + '\CIR_composites.')
