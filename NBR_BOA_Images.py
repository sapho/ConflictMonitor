...
def callNBR(nir,swir,filepath,image,resolution,nameDatum,band):
    ...
    outPath = os.path.join(filepath,image, 'IMG_DATA', resolution, nameDatum + 'NBR.tif')
...
def nbrOldNew(nbrDict,filepath,resolution):
    print("in nbrOldNew")
    for key in nbrDict:
        if(key != len(nbrDict)):
            ...
            outPath = os.path.join(filepath,'results',nameDatum + nameDatum2 + 'result_NBR.tif')
            newFile = CreateGeoTiff(outPath,result,driver,xsize,ysize,GeoT,Projection,DataType)
        else:
...
filepath2 = os.path.join(root,'data','results')
...
nbrOldNew(nbrDict,filepath,"R10m")
#for image2 in arr2:
